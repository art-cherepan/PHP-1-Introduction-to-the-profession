...
require_once __DIR__ . '/ReviewProduct.php';
...
class Product
{
    ...
    {
        $select = 'SELECT * FROM reviews_products WHERE id_product=:id ORDER BY date DESC';
        $reviews = $this->DB->query($select, [':id' => $this->id]);
        if ($reviews != false) {
            foreach ($reviews as $review) {
                $this->reviews[] = new ReviewProduct($review['id'], $review['id_user'], $review['id_product'], $review['date'], $review['text']);
            }
            return true;
        } else {
            return false;
